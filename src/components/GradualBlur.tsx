...
const mergeConfigs = (...configs: Record<string, unknown>[]): Record<string, unknown> =>
...
const useResponsiveDimension = (responsive: boolean, config: Record<string, unknown>, key: string): unknown => {
...
const useIntersectionObserver = (ref: React.RefObject<HTMLElement | null>, shouldObserve: boolean = false): boolean => {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= (config.divCount as number); i++) {
      ...
      if (config.exponential) {
        ...
        blurValue = 0.0625 * (progress * (config.divCount as number) + 1) * (currentStrength as number);
      }
